...
컬렉션을 정의되었다면, 이제 조작하는 방법을 배워봅시다!

## Isar 열기

제일 먼저 Isar 인스턴스가 필요합니다. 각 인스턴스에는 데이터베이스 파일을 저장할 쓰기 권한이 있는 디렉토리가 필요합니다. 디렉토리를 지정하지 않는 경우, Isar는 현재 플랫폼에 적합한 기본 디렉토리를 찾습니다.

Isar 인스턴스에서 사용하고 싶은 모든 스키마를 지정해주세요. 여러 인스턴스를 열고 있는 경우에도 각각의 인스턴스에 같은 스키마를 지정해야 합니다.
...
| 속성              | 설명                                                                                                                                                                                                                                |
| ------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `name`              | 인스턴스들에 이름을 지정해서 엽니다. 기본적으로 `"default"`가 사용됩니다.                                                                                                                                                                 |
...
| `maxSizeMib`        | 데이터베이스 파일의 최대 크기(MiB)입니다. Isar는 무한하지 않은 가상 메모리를 사용하므로 이 값은 신중하게 결정해야 합니다. 여러 인스턴스를 열면 사용 가능한 가상 메모리가 공유되므로 각 인스턴스의 `maxSizeMib` 가 더 작아집니다. 기본값은 2048 입니다. |
...
| `compactOnLaunch`   | 인스턴스를 열 때 데이터베이스를 압축하는지 여부를 선택합니다.                                                                                                                                                               |
| `inspector`         | 디버그 빌드에서 Inspector 를 사용하도록 설정합니다. 프로파일이나 릴리즈 빌드에서는 이 옵션이 무시됩니다.                                                                                                                              |
...
모든 컬렉션들은 Isar 인스턴스 안에 들어있습니다. 레시피 컬렉션은 아래처럼 가져옵니다:

```dart
final recipes = isar.recipes;
```

너무 쉽습니다! 컬렉션 접근자를 사용하기 싫다면, `collection()` 메서드를 사용해도 됩니다.
...
아직 컬렉션에 데이터가 들어있지 않지만, 아이디가 `123` 인 가상의 객체가 있다고 가정하고 가져오겠습니다.

```dart
final recipe = await isar.recipes.get(123);
```

`get()` 은 객체를 `Future` 로 반환하고, 해당 객체가 존재하지 않는 경우에는 `null` 을 반환합니다. 모든 Isar 작업들은 기본적으로 비동기적으로 작동합니다. 대부분 동기적인 방법도 가지고 있습니다.
